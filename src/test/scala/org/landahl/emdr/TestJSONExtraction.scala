...
class TestJSONExtraction extends FunSuite {
  ...
  test("extract order1") {
    ...
    uudif.currentTime should equal(dateFormatter.parse("2011-10-22T15:46:00+00:00"))
    uudif.rowsets.size should equal(3)

    val rs0 = uudif.rowsets(0)
    rs0.generatedAt should equal(dateFormatter.parse("2011-10-22T15:43:00+00:00"))
    ...
    rs0.rows(0) match {
      case row: OrderRow => {
        ...
        row.issueDate should equal(dateFormatter.parse("2011-12-03T08:10:59+00:00"))
    ...
    rs0.rows(1) match {
      case row: OrderRow => {
        ...
        row.minVolume should equal(1)
        row.bid should equal(false)
        row.issueDate should equal(dateFormatter.parse("2011-12-03T10:53:26+00:00"))
        row.duration should equal(90)
    ...
    rs2.generatedAt should equal(dateFormatter.parse("2011-10-22T15:43:00+00:00"))
  ...
  test ("extract history1") {
    val uudif = JsonToUUDIF.extract(SampleData.history1)
    ...
    val rs0 = uudif.rowsets(0)
    rs0.generatedAt should equal(dateFormatter.parse("2011-10-22T15:42:00+00:00"))
    rs0.regionID should equal(Some(10000065))
    rs0.typeID should equal(11134)
    ...
    rs0.rows(0) match {
      case row: HistoryRow => {
        row.date should equal(dateFormatter.parse("2011-12-03T00:00:00+00:00"))
